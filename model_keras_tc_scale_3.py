...
class KPLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self._kp_kernel_conv is None:
            self._kp_kernel_conv = tf.Variable(np.zeros((3,3,config.NUM_KP,2048)), dtype=tf.float32, name='KPLayer_kernel')
        if self._kp_bias is None:
            self._kp_bias = tf.Variable(np.zeros((1,1,config.NUM_KP)), dtype=tf.float32, name='KPLayer_bias')

    def call(self, x):        
        z = tf.nn.conv2d_transpose(x, self._kp_kernel_conv, [x.shape[0], x.shape[1] * 3, x.shape[2] * 3, config.NUM_KP], [1, 3, 3, 1], padding='VALID')
        z = z + self._kp_bias
        z = tf.math.sigmoid(z)
        if self._debug:
...
class ShortOffsetLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self._kp_kernel_conv is None:
            self._kp_kernel_conv = tf.Variable(np.zeros((3,3, 2 * config.NUM_KP,2048)), dtype=tf.float32, name='ShortOffsetLayer_kernel')
        if self._kp_bias is None:
            self._kp_bias = tf.Variable(np.zeros((1,1,2 * config.NUM_KP)), dtype=tf.float32, name='ShortOffsetLayer_bias')
        
    def call(self, x):
        z = tf.nn.conv2d_transpose(x, self._kp_kernel_conv, [x.shape[0], x.shape[1] * 3, x.shape[2] * 3, 2 * config.NUM_KP],[1, 3, 3, 1], padding='VALID')
        z = z + self._kp_bias
        if self._debug:
...
class MiddleOffsetLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self._kp_kernel_conv is None:
            self._kp_kernel_conv = tf.Variable(np.zeros((3,3, 4 * config.NUM_EDGES,2048)), dtype=tf.float32, name='MiddleOffsetLayer_kernel')
        if self._kp_bias is None:
            self._kp_bias = tf.Variable(np.zeros((1,1,4 * config.NUM_EDGES)), dtype=tf.float32, name='MiddleOffsetLayer_bias')
        
    def call(self, x):        
        z = tf.nn.conv2d_transpose(x, self._kp_kernel_conv, [x.shape[0], x.shape[1] * 3, x.shape[2] * 3, 4 * config.NUM_EDGES], [1, 3, 3, 1], padding='VALID')
        z = z + self._kp_bias
        if self._debug:
...
class LongOffsetLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self._kp_kernel_conv is None:
            self._kp_kernel_conv = tf.Variable(np.zeros((3,3, 2*config.NUM_KP,2048)), dtype=tf.float32, name='LongOffsetLayer_kernel')
        if self._kp_bias is None:
...
class SemSegLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self._kp_kernel_conv is None:
            self._kp_kernel_conv = tf.Variable(np.zeros((3,3, 1,2048)), dtype=tf.float32, name='SemSegLayer_kernel')
        if self._kp_bias is None:
            self._kp_bias = tf.Variable(np.zeros((1,1,1)), dtype=tf.float32, name='SemSegLayer_bias')

    def call(self, x):        
        z = tf.nn.conv2d_transpose(x, self._kp_kernel_conv, [x.shape[0], x.shape[1] * 3, x.shape[2] * 3, 1], [1, 3, 3, 1], padding='VALID')
        z = z + self._kp_bias
        z = tf.math.sigmoid(z)
