...
def get_instance_masks(skeletons, seg_mask, long_offsets, threshold=True):
    ...
    p_i, p_j = np.nonzero(seg_mask[:,:,0]>0.5)
    n = len(p_i)
...
def get_skeletons_and_masks_from_batch(outputs_batch):
    kp_maps_batch, short_offsets_batch, mid_offsets_batch, long_offsets_batch, seg_mask = outputs_batch

    skeletons_batch = []
    instance_masks_batch = []
    for kp_maps, short_offsets, mid_offsets, long_offsets, seg_mask in zip(kp_maps_batch, short_offsets_batch, mid_offsets_batch, long_offsets_batch, seg_mask):
        heatmaps = compute_heatmaps(kp_maps, short_offsets)
        for i in range(config.NUM_KP):
            heatmaps[:,:,i] = gaussian_filter(heatmaps[:,:,i], sigma=2)
        pred_kp = get_keypoints(heatmaps)
        skeletons = group_skeletons(pred_kp, mid_offsets)
        instance_masks = get_instance_masks(skeletons, seg_mask, long_offsets)
        skeletons_batch.append(skeletons)
        instance_masks.append(instance_masks)

    return np.stack(skeletons_batch, axis=0), np.stack(instance_masks_batch, axis=0)
